  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      edges.forEach((e) => {
        ...
        ctx.strokeStyle = `rgba(246,166,35,${alpha})`;
        ctx.lineWidth = 1 * dpr;
      ...
      pulses.forEach((p) => {
        ...
        ctx.fillStyle = "rgba(52,217,168,.95)";
        ctx.beginPath(); ctx.arc(x, y, 2.5 * dpr, 0, 7); ctx.fill();
        ctx.shadowBlur = 12 * dpr; ctx.shadowColor = "rgba(52,217,168,.85)";
        ctx.beginPath(); ctx.arc(x, y, 2.5 * dpr, 0, 7); ctx.fill();
        ctx.shadowBlur = 0;
      });
      nodes.forEach((n) => {
        ...
        ctx.fillStyle = light ? "rgba(246,166,35,.5)" : "rgba(255,210,140,.7)";
